                ...
                <div class="bg-gray-50 rounded-lg p-4 mb-4">
                    <div class="space-y-2">
                        ...
                        <div class="flex justify-between text-sm">
                            ...
                            <span class="font-semibold text-red-600">{{ number_format($facture->montant_restant, 0, ',', ' ') }} FCFA</span>
                        </div>
                        <div class="border-t pt-2 mt-2">
                            @if($facture->lignes && is_array($facture->lignes))
                            <p class="text-xs text-gray-500 mb-1">Détails:</p>
                            @foreach($facture->lignes as $ligne)
                            <div class="flex justify-between text-xs text-gray-600">
                                <span>{{ $ligne['designation'] ?? 'Item' }}</span>
                                <span>{{ number_format($ligne['montant'] ?? 0, 0, ',', ' ') }} FCFA</span>
                            </div>
                            @endforeach
                            @endif
                        </div>
                ...
                <div class="bg-green-50 border border-green-200 rounded-lg p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            @if($facture->paiements->isNotEmpty())
                            <p class="text-sm text-green-700">
                                <span class="font-semibold">Date de paiement:</span> {{ $facture->paiements->last()->date_paiement ? $facture->paiements->last()->date_paiement->format('d/m/Y à H:i') : 'N/A' }}
                            </p>
                            <p class="text-sm text-green-700">
                                <span class="font-semibold">Méthode:</span> {{ $facture->paiements->last()->methode_paiement }}
                            </p>
...
<div class="bg-white rounded-lg shadow-sm p-4 mb-6">
    <div class="flex flex-wrap gap-4">
        <div class="flex-1 min-w-[200px]">
            ...
            <select class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500" onchange="window.location.href=this.value">
                <option value="{{ route('patient.factures') }}">Tous les statuts</option>
                <option value="{{ route('patient.factures', ['statut' => 'IMPAYEE']) }}" {{ request('statut') == 'IMPAYEE' ? 'selected' : '' }}>En attente</option>
                <option value="{{ route('patient.factures', ['statut' => 'PAYEE']) }}" {{ request('statut') == 'PAYEE' ? 'selected' : '' }}>Payée</option>
                <option value="{{ route('patient.factures', ['statut' => 'ANNULEE']) }}" {{ request('statut') == 'ANNULEE' ? 'selected' : '' }}>Annulée</option>
            </select>
        </div>
                ...
                <div class="bg-green-50 border border-green-200 rounded-lg p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm font-semibold text-green-800 mb-1">✓ Facture payée</p>
                            <p class="text-sm text-green-700">
                                <span class="font-semibold">Date de paiement:</span> {{ $facture->paiements->isNotEmpty() && $facture->paiements->last()->date_paiement ? $facture->paiements->last()->date_paiement->format('d/m/Y à H:i') : 'N/A' }}
                            </p>
                            <p class="text-sm text-green-700">
                                <span class="font-semibold">Méthode:</span> {{ $facture->paiements->isNotEmpty() ? $facture->paiements->last()->methode_paiement : 'N/A' }}
                            </p>
                        </div>
                        <a href="{{ route('patient.facture.show', $facture) }}" target="_blank" class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg text-sm flex items-center">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <div class="flex items-center justify-between">
            <div>
                <p class="text-sm text-gray-500">En attente</p>
                <p class="text-2xl font-bold text-yellow-600">{{ $factures->where('statut', 'IMPAYEE')->count() }}</p>
            </div>
            <div class="bg-yellow-100 rounded-full p-3">
                <svg class="w-6 h-6 text-yellow-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
